...
def random_adjust(image):
    strength = np.random.uniform(low=-0.5, high=0.0)
    bright_enhancer = ImageEnhance.Brightness(image)
    image = bright_enhancer.enhance(1+strength)
    # bright_img.save(filea2)
    strength = np.random.uniform(low=-0.5, high=0.0)
    contrast_enhancer = ImageEnhance.Contrast(image)
    image = contrast_enhancer.enhance(1+strength)
    strength = np.random.uniform(low=-0.5, high=0.0)
    color_enhancer = ImageEnhance.Color(image)
    image = color_enhancer.enhance(1+strength)
...
class ImageToImage2D(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # plt.imshow(mask)
        # plt.show()
        mask[mask<=5] = 0  #  10 or 20   for drive STARE chasedb1
        mask[mask>5] = 1
        # mask[mask <= 150] = 0  # 10 or 20   for  cornea
        # mask[mask > 150] = 1
        # mask[mask <= 0] = 0  #mumoseg
        # mask[mask > 0] = 1

...
class WeightedDiceBCE(nn.Module):
    ...
    def _show_dice(self, inputs, targets):
      ...
      #################################
        #
        # predsi = torch.cat([inputs[0], inputs[0]], 0)
        # predsi = torch.cat([inputs[0], predsi], 0)
        # predsi = predsi.cpu().detach().numpy()  # to cv2处理
        # predsi = predsi * 255  # normalize，将图像数据扩展到[0,255]
        #
        # predsi = np.array(predsi, dtype='uint8')
        #
        # predsi = predsi.transpose(1, 2, 0)  # mat_shape: (224, 224，3)
        # # predsi = cv2.cvtColor(predsi, cv2.COLOR_BGR2GRAY)
        #
        # plt.imshow(predsi)
        # plt.show()
        #
        # maski = targets[0].cpu().detach().numpy()
        # maski = maski * 255
...
def F1sesp_on_batch(te_mask, predictions):
    ...
    return F1_score, precision, sensitivity, specificity,accuracy

...
from nets.CSCM_CCA_Net import CSCM_CCA_Net
...
def main_loop(batch_size=config.batch_size, model_type='', tensorboard=True):
    ...
    if model_type =='CSCM_CCA_Net' :#:'UNet'SRAUnet
        config_vit = config.get_CTranS_config()
        # model = CSCM_CCA_Net( )   #ultri-mamba
        model = CSCM_CCA_Net(config_vit,n_channels=config.n_channels,n_classes=config.n_labels)
        #model = AMSUnet(config_vit,n_channels=config.n_channels, n_classes=config.n_labels)

    elif model_type == 'SRAUnet_pretrain':
        config_vit = config.get_CTranS_config()
...
##################################################
    ...
    if config.cosineLR is True:
        lr_scheduler = CosineAnnealingWarmRestarts(optimizer, T_0=10, T_mult=1, eta_min=3e-5)
    else:
        lr_scheduler =  None
    if tensorboard:
